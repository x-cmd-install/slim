# slim

[English version](./README.md)

Slim(toolkit): Don't change anything in your container image and minify it by up to 30x (and for compiled languages even more) making it secure too! (free and open source)

[![x-cmd/install — slim Code Quality Monitoring Repo Card](https://x-cmd.com/repo-card/slim.svg?lang=zh)](https://x-cmd.com/install/slim)

## 安装

```sh
x install slim
```

## 代码洞察

合计: **1,370,114** 行代码（覆盖前 5 种语言、共 **5017** 个文件）。

| 语言 | 代码 | 注释 | 空行 | 文件数 |
|------|-----:|-----:|-----:|------:|
| Go | 1,322,746 | 206,312 | 124,189 | 4864 |
| Yaml | 20,312 | 219 | 451 | 23 |
| AssemblyGAS | 11,203 | 851 | 2,529 | 56 |
| Protobuf | 7,711 | 14,660 | 4,093 | 67 |
| Json | 5,184 | 0 | 25 | 7 |

## OpenSSF Scorecard 评分

总评分: **3.8 / 10**

评分最低的几项:

- **Packaging** (-1/10) — packaging workflow not detected
- **Token-Permissions** (-1/10) — No tokens found
- **Dangerous-Workflow** (-1/10) — no workflows found

## 源代码

- **上游仓库**: <https://github.com/slimtoolkit/slim>
- **许可证**: Apache-2.0

## 发布

- **最新版本**: `1.40.11` (2024-02-02)
- **最近提交**: 2026-09-19
- **Release 含资产**: 5 个

## 流行度

- **Star**: 23,416 · **Fork**: 842 · **开放 issue**: 351 · **贡献者**: 68

## 累计统计

- **发布数**: 49 · **已合并 PR**: 272 · **开放 PR**: 25 · **已关闭 issue**: 163 · **开放 issue**: 188 · **提交数**: 1140

## 最近活动

| 时间窗口 | 起始 | 发布 | 已合并 PR | 开放 PR | 已关闭 issue | 开放 issue | 提交 |
|---|---|---:|---:|---:|---:|---:|---:|
| 30d | 2026-08-27 | 0 | 1 | 2 | 0 | 0 | 1 |
| last60d | 2026-07-28 | 0 | 1 | 3 | 0 | 1 | 2 |
| 90d | 2026-06-28 | 0 | 2 | 4 | 1 | 1 | 2 |
| last180d | 2026-03-30 | 0 | 2 | 7 | 2 | 2 | 2 |
| 360d | 2025-10-01 | 0 | 23 | 12 | 4 | 8 | 25 |
| last720d | 2024-10-06 | 0 | 53 | 18 | 11 | 19 | 58 |

## Release 资产

| 资产 | 大小 | 目标平台 |
|------|-----:|----------|
| [dist_linux.tar.gz](https://github.com/slimtoolkit/slim/releases/download/1.40.11/dist_linux.tar.gz) | 26.9 MiB | `native/unknown` |
| [dist_linux_arm.tar.gz](https://github.com/slimtoolkit/slim/releases/download/1.40.11/dist_linux_arm.tar.gz) | 25.1 MiB | `native/linux/arm` |
| [dist_linux_arm64.tar.gz](https://github.com/slimtoolkit/slim/releases/download/1.40.11/dist_linux_arm64.tar.gz) | 24.6 MiB | `native/linux/arm64` |
| [dist_mac.zip](https://github.com/slimtoolkit/slim/releases/download/1.40.11/dist_mac.zip) | 70.3 MiB | `other` |
| [dist_mac_m1.zip](https://github.com/slimtoolkit/slim/releases/download/1.40.11/dist_mac_m1.zip) | 65.8 MiB | `other` |

## 改进这些数据

slim 的安装元数据由 [x-cmd/install](https://github.com/x-cmd/install) 索引维护——这是一份由 x-cmd 在安装时读取的精选 YAML 包列表。如果 `slim` 缺失、过期，或安装行为有问题，欢迎在该 repo 提 issue 或 PR：

- **提交 issue**: <https://github.com/x-cmd/install/issues/new>
- **编辑包条目**: <https://github.com/x-cmd/install/edit/main/slim.yml>（或索引实际使用的路径）

本页面的数据（card / loc / scorecard / release）由 [x-cmd-install-action](https://github.com/x-cmd-install/x-cmd-install-action) 自动采集，每日重新生成。**安装行为**（版本选择、平台差异、依赖处理）的改进应提交到上游索引。

_数据快照: `data/card/260926.yml` · 2026-09-26T04:48:18Z._
